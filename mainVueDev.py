...
oauthws = OAuth2PasswordBearer(tokenUrl = "/login")
...
class ConnectionManager:
    ...
    async def get_cur_chat_users(self,chat_id,user):
        active = []
...
async def everything_user_related(websocket: WebSocket):
    await websocket.accept()
...
async def websocket_endpoint(websocket: WebSocket,chat_id : str):    
    ...
    try:
        while True:
            ...
            if other_dude not in await manager.get_cur_chat_users(chat_id,user):
        ...
        if line_number:
            print(f"The error occurred at line {line_number}.")
